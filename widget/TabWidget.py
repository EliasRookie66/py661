...
class A661TabWidget(QTabWidget):
    ...
    def showContextMenu(self, pos, tab_index):
        menu = QMenu(self)
        open_in_server_action = menu.addAction(self.tr("Open In Server"))
        menu.addSeparator()
        create_UACDS_interface_action = menu.addAction(self.tr("Create UACDS Interface"))
        open_UACDS_interface_action = menu.addAction(self.tr("Open UACDS Interface"))
        menu.addSeparator()
        ...
        close_other_action = menu.addAction(self.tr("Close Other"))
        close_other_action.setEnabled(False)

        action = menu.exec_(self.mapToGlobal(pos))
        if action == open_in_server_action:
            print('open_in_server_action is triggered')
            self.server = ServerWindow(self.tr('server'),  self.findChild(A661Label), udp_port=5000, target_port=5001)
            self.server.show()
